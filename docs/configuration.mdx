...
### Persisted OAuth Credential Settings

| Variable | Description |
|----------|-------------|
| `ATLASSIAN_OAUTH_TOKEN_PROFILE` | Local persisted-token slot (default: `default`); use a distinct label for each Atlassian account |
| `JIRA_OAUTH_TOKEN_PROFILE` | Jira-specific token profile; overrides `ATLASSIAN_OAUTH_TOKEN_PROFILE` |
| `CONFLUENCE_OAUTH_TOKEN_PROFILE` | Confluence-specific token profile; overrides `ATLASSIAN_OAUTH_TOKEN_PROFILE` |

The profile label is part of the persisted credential's storage identity. It is
not an Atlassian-verified principal identifier and does not change OAuth scopes.
Use the same profile during `--oauth-setup` and normal server startup.
